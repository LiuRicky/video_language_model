...
from models.blip_video_retrieval import blip_video_retrieval
import utils
...
def evaluation(model, data_loader, tokenizer, device, config):
    ...
    for video, video_id in data_loader: 
        ...
        # video_embed = model.temporal_transformer(video_feat[:,0,:].view(B,N,-1))
        # video_embed = model.vision_proj(video_embed).mean(dim=1)
    ...
    # ### t2v dsl ###
    # sims_matrix_original = copy.deepcopy(sims_matrix)
    # sim_matrix = sim_matrix * softmax(sim_matrix, axis=0)
    # ### t2v dsl ###
...
def main(args, config):
    ...
    train_dataset, val_dataset, test_dataset = create_dataset('video_retrieval', config)  
    ...
    model = blip_video_retrieval(pretrained=config['pretrained'], image_size=config['image_size'], vit=config['vit'], 
    ...
    for epoch in range(0, config['max_epoch']):    
        ...
        score_val_i2t, score_val_t2i = evaluation(model_without_ddp, test_loader, model_without_ddp.tokenizer, device, config)
        np.save('retrieval_result_token_mix.npy', score_val_t2i)
        score_test_i2t, score_test_t2i = score_val_i2t, score_val_t2i # evaluation(model_without_ddp, test_loader, model_without_ddp.tokenizer, device, config)
