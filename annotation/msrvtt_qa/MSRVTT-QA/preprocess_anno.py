...
def get_vocabulary(train_json, save=False):
    ...
    with open("answer_list.json", "w") as ans_file:
        json.dump(ans_list, ans_file)
    return vocab
...
def json_to_df(vocab, train_json, val_json, test_json, save=False):
    ...
    val_df = val_df[
        val_df["answer"].isin(vocab)
    ]  # do not use val samples of which the answer is not in the vocab
