...
class VideoTrecvidDataset(Dataset):

    def __init__(self, video_root, ann_root, num_frm=4, frm_sampling_strategy="rand", max_img_size=384, video_fmt='.webm'):
        '''
        image_root (string): Root directory of video
        ann_root (string): directory to store the annotation file
        '''        
        filename = 'test_topics.json'

        # download_url(url,ann_root)
        self.annotation = json.load(open(os.path.join(ann_root,filename)))
        ...
        self.text = [pre_caption(ann['captions'],40) for ann in self.annotation]
        self.txt2video = [i for i in range(len(self.annotation))]
        self.video2txt = self.txt2video    

        video_file_name = 'valid_videos.json'
        self.videos = json.load(open(os.path.join(ann_root,video_file_name)))
    ...
    def _load_video_from_path_decord(self, video_path, height=None, width=None, start_time=None, end_time=None, fps=-1):
        try:
            ...
            # return None
            raw_sample_frms = torch.zeros((self.num_frm, height, width, 3))
